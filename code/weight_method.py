...
def calculate_accuracy(predicted_matrix, true_interaction_matrix, threshold=0.5):
    def threshold_predictions(predictions, threshold):
        return (predictions > threshold).int()
    binary_predictions = threshold_predictions(predicted_matrix, threshold)
    accuracy = (binary_predictions == true_interaction_matrix).float().mean().item()
    return accuracy